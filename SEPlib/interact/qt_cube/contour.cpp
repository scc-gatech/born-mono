...
(void) ov; // quiet compiler about unused argument
  ...
  dy=axes[0].d;
   ...
   int b1u=f1+0*bb1, b2u=f2+0*bb2, e1u=f1+n1+0*ee1,e2u=f2+n2+0*ee2;
...
void contour::Paintcontours(int n2, int n1, QPainter *painter, float *buf,
  float minv, float maxv){
     ...
     if(dc_set) samp=dc;
       ...
       optimal_scale(num_lines,beg,maxv,&beg,&samp);
     }
...
void contour::get_shifted_image_pos_fract(float fx, float fy,int *id1, int *id2){
 ...
 box::box(int ix, int iy, QString txt,  float ang,QFontMetrics *fm){
   ...
   int pw = fm->width( txt);
     ...
     shiftx=shifty=0;
     int len=(int)(pw*fabs(cos(ang))+ph*fabs((sin(ang)))*1.2+ph*1.5);
     int het=(int) (pw*fabs(sin(ang))+ph*fabs((cos(ang)))*1.2+ph*1.5);

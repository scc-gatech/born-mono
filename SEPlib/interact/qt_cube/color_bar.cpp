...
color_bar::color_bar(user_color *col, QWidget* parent,  const char* name){
	pm=QPixmap(100,100);
	if(parent==0 && name==0){}
	color="red";
...
void color_bar::to_pt(QString col,float px, float py, int *ix, int *iy){
  *ix=(int)(W*1.*px);
   *iy=(int)(nh[col]-nh[col]*py)+bh[col];
}
void color_bar::to_pct(QString col,int ix, int iy, float *px, float *py){
  *px=(ix)/1./W;
  //*py=(H-iy)/H;
  *py=(float)(nh[col]-(iy-bh[col] )   )/(float)nh[col];
  
}
void color_bar::paintEvent( QPaintEvent *e ){
...
void color_bar::resizeEvent( QResizeEvent *e ){
 if(e==0){}
...
int *color_bar::return_map(QString col){
  ...
  fprintf(stderr,"Found 1 point \n");
    ...
    while(ib<ie){
      ...
      if(ia<=ie){
        ...
        //assert(ie==ia);
        float d=(float)(MIN(ia*8,256)-ib*8);
        for(int i=ib*8; i < MIN(ia*8,256); i++){
...
void color_bar::set_source(QString text){
  for(int i=0; i < 33; i++){
...
void color_bar::load_cbar(QString text){
...
void color_bar::save_cbar(QString text){
...
void color_bar::extract_draw(QString col, QPainter *painter){
...
QString color_bar::find_col_panel(int ih){
...
void color_bar::mouseReleaseEvent(QMouseEvent *e){
  set_sizes();
...
void color_bar::del_pt(int ix, int iy){
  float px,py;
...
void color_bar::add_pt(int ix, int iy){
  float px,py;
  QString col=find_col_panel(iy);
  if(col!="none"){
    color=col;

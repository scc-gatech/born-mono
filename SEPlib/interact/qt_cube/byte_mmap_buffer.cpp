#include "byte_mmap_buffer.h"


//Initialize and read in buffer
 byte_mmap_buffer::byte_mmap_buffer(util *p, hypercube *h,io_func *i,int in,int *nwbuf, int *fwbuf){
 ...
 void byte_mmap_buffer::read_buffer(int *nwbuf, int *fwbuf,int *nwio, int *fwio, int ndim ,int *nloop){
...
// if(tflt!=0) delete [] tflt;
 ...
 unsigned char *byte_mmap_buffer::get_char_data(orient_cube *pos, int iax1, int f1, int e1,
  int iax2, int f2, int e2){
      ...
      if(!hold[iax1] || !hold[iax2]) {
         ...
         if(pos->rotate && (!hold[pos->rot_ax[0]] || !hold[pos->rot_ax[1]]){
      ...
      long long *index=form_index_map_new(pos,iax1,iax2,f1>e1,f2>e2);
...
float *byte_mmap_buffer::get_float_data(orient_cube *pos, int iax1, int f1, int e1, int iax2,
    int f2, int e2){
 ...
 void byte_mmap_buffer::calc_histo(){
 ...
 float byte_mmap_buffer::get_value(orient_cube *pos){

...
std::vector<path> auto_brown::return_path(std::vector<int> locs,float_2d *array){
  ...
  int imid=(array->get_axis(1).n-1)/2;
 n1=imid*2+1;
  float mymin=array->my_min();
  for(int i=0; i < array->get_n123(); i++){
    array->vals[i]=1./(.3+2.5*(array->vals[i]-mymin));
  }
    n2=array->get_axis(2).n; n3=1; 

  slow=array->vals;
  
  axis a1=array->get_axis(1),a2=array->get_axis(2);
  float_2d tot=float_2d(a1,a2),times=float_2d(a1,a2);
  tot.zero();
  b1=1; b2=1; b3=1;
  d1=1.; d2=1.; d3=1.;
  n12=n1*n2;
...
for(int isrc=0; isrc< (int)locs.size(); isrc++){
  alloc_local();
   ...
   calc_time(imid,locs[isrc],0,times.vals);
   int i=0;

 ...
 std::map<int,picks_vec* > pick_bucket::return_picks_from_range(int ip,std::vector<int> keys,int *iloc){
  ...
  for(int i=0; i < 8; i++){ 
     if(iloc[i]==-1 && i!=ip){
      ...
      if(ip!=i){
        buck_base+=block_b[i]*iloc[i]/BUCKET_SIZE;
      }
  }
  }
...
pick_new *pick_bucket::add_pick(int *iloc,long long p, int te, QString col, int ex){
 
  pick_new *pp=buckets[bucket_num(p)]->add_pick(iloc,p,te,col,ex);
  return pp;
}

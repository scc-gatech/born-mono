...
#define CALLARGS_SHADOW \
  /* unpack structure attributes into argument list */\
  (DataBuffer(data),render->image,render->shadow,render->zbuffer,\
  render->cmap,render->tmap,render->wide,render->hite,v0,h0,\
  (int)(render->fence_transp*DataValueSize(data)*.01),\
  MapSize(hmap),MapVec(hmap),MapInterp(hmap),AxisStride_s(MapAxis(hmap)),\
  MapSize(vmap),MapVec(vmap),MapInterp(vmap),AxisStride_s(MapAxis(vmap)),\
  MapSize(zmap),MapFrame(zmap),AxisStride_s(MapAxis(zmap)),\
  AxisDir(MapAxis(zmap)),MapInverse(zmap,MapFrame(zmap)),\
  MapFrame(map4),AxisStride_s(MapAxis(map4)),\
  MapFrame(map5),AxisStride_s(MapAxis(map5)),\
  attr->skew)

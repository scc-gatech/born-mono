...
static float    alpha = 0.4375f;
static float    beta = 0.1875f;
static float    gamma = 0.3125f;
static float    delta = 0.0625f;
...
void dithline (unsigned char *inpline, unsigned char *outline, int npixels, int linenum, int imethod)
{
int             greydata;
...
/* Random Dither */
	...
	for (i1 = 0; i1 < npixels; i1++)
	{
	    ...
	    if (greydata > irand)
	    {
		outline[i1] = (unsigned char) pix_off;
	    }
	    else
	    {
		outline[i1] = (unsigned char) pix_on;
	    }
	}
	break;

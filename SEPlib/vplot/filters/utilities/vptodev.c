 ...
 *	preserving area.
 * Stewart A. Levin (SEP), Feb 26 2015
 *      Added conversion to user coordinates for useful getapoint()
 *      output.
 */
...
void vptodevw (int x1, int y1, int x2, int y2, int *x1out, int *y1out, int *x2out, int *y2out)
{
...
void devtovpuxy (int x, int y, float *outux, float *outuy)
{
float           tempx, tempy, temp;
    ...
    *outux = (tempx/RPERIN - uxorigin)/uxscale;
    *outuy = (tempy/RPERIN - uyorigin)/uyscale;
}

void devtovpw (int x1, int y1, int x2, int y2, int *x1out, int *y1out, int *x2out, int *y2out)
{
int             x11, y11, x12, y12, x21, y21, x22, y22, a, b;

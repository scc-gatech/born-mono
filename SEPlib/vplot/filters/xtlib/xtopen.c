 ...
 * Stewart A. Levin (SEP) Aug 24 2012
 *      Added hook to set _Xdebug to slow plotting for visual progress
 */
...
static void choose_visual( Display *display, int screen, int max_depth, XVisualInfo *vinfo );
...
char ** myxargv;
  ...
  bcopy ((char *) sepxargv, (char *) myxargv, myxargc * sizeof (char *));
    ...
    vartemp.i = &depth;
    if(0==getpar ("depth", "d", vartemp)) depth=app_data.vis_depth;
    /* Choose a visual */
...
/*    ERR( COMMENT,name,"visual depth = %d",visual_depth);*/
    ...
    vartemp.i = &tellme_resolution;
    getpar ("x_screen_info", "1", vartemp);
    ...
    vartemp.i = &num_col_req;
    if ( !GETPAR("numcol","d",vartemp) ) num_col_req = app_data.num_col;
...
/*    XtSetArg (args[cnt], XtNargc, myxargc); ++cnt;*/
    ...
    vartemp.i = &want_buttons;
    if( !GETPAR("buttons","1",vartemp) ) want_buttons = app_data.buttons;
    vartemp.i = &want_labels;
    if( !GETPAR("labels","1",vartemp) ) want_labels = app_data.labels;
    vartemp.i = &want_text;
    if( !GETPAR("want_text","1",vartemp) ) want_text = app_data.textpane;

    vartemp.i = &xt_stretchy;
    if( !GETPAR("stretchy","1",vartemp) ) xt_stretchy=app_data.stretchy;

    vartemp.i = &boxy;
    if( !GETPAR("boxy","1",vartemp) ) boxy = 0;

    vartemp.i = &see_progress;
    if( !GETPAR("see_progress","1",vartemp) ) see_progress=0;
    vartemp.i = &want_images;
    if( !GETPAR("images","1",vartemp) ) want_images = app_data.images;
    vartemp.i = &greedy_pixmaps;
    if( !GETPAR("pixmaps","1",vartemp)) greedy_pixmaps=app_data.pixmaps;
    ...
    if( want_text ) {
        vartemp.s = &(message_text[0]);
        if( ! GETPAR("message","s",vartemp) ) strcpy(message_text,"");

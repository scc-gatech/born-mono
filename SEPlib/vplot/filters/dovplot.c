 ...
 */
	...
	{
	    dev.close (CLOSE_FLUSH);
	    message (MESG_ERASE,dummymsg);
	    message (MESG_ON,dummymsg);
	    message (MESG_HOME,dummymsg);
	    message (MESG_READY,dummymsg);
	    message (MESG_HIGHLIGHT_ON,dummymsg);
	    message (MESG_TEXT, "Type Return to Continue...  ");
	    message (MESG_DONE,dummymsg);
	    ...
	    message (MESG_HIGHLIGHT_OFF,dummymsg);
	    if (!allowecho)
	    {
		message (MESG_READY,dummymsg);
...
/*    fprintf(stderr,"in dovplot c\n");*/
	    break;
	case VP_UORIGSCL: /* capture user origin & scale in vplot generator */
	    uxorigin = getf(pltin);
	    uyorigin = getf(pltin);
	    uxscale = getf(pltin);
	    uyscale = getf(pltin);
...
/*    fprintf(stderr,"in dovplot m\n");*/
		...
		else
		{
		    ...
		    angle = 3.1415926535f * pat[ipat].ydim / 180.0f;
...
/*    fprintf(stderr,"in dovplot ai\n");*/
		...
		j = k * (patternmult * pixels_per_inch / RPERIN) * 0.5f;
		if (j < 1)
		    j = 1;
		for (i = 0; i < nx * 2; i++)
		{
...
/*fprintf(stderr,"in this read 1 \n");*/
				READ_RASTER (
					     rasterline2[j] = (unsigned short) GREY_MAP (ras_offset + (unsigned short) fgetc (pltin)),
					     rasterline2[j] = (unsigned short) GREY_MAP (ras_offset + (unsigned short) fgetc (pltin)*256+fgetc(pltin)),
					     rasterline2[j + jj] =(unsigned short)  GREY_MAP (ras_offset * ((ibyte & (001 << (7 - j))) != 0))
				 );
			    }
			    else
			    {
/*fprintf(stderr,"in this read 2 \n");*/
				READ_RASTER (
					     rasterline2[j] = (unsigned short) COLOR_MAP (ras_offset + (unsigned short) fgetc (pltin)),
					     rasterline2[j] = (unsigned short) COLOR_MAP (ras_offset + (unsigned short) fgetc (pltin)*256+fgetc(pltin)),
					     rasterline2[j + jj] = (unsigned short) COLOR_MAP (ras_offset * ((ibyte & (001 << (7 - j))) != 0))
				 );
			    }
			    for (j = 0; j < num_pat; j++)
			    {
...
/*fprintf(stderr,"in this read 3 \n");*/
				READ_RASTER (
					     rasterline[pos + j] = (unsigned short) GREY_MAP (ras_offset + (unsigned short) fgetc (pltin)),
					     rasterline[pos + j] = (unsigned short) GREY_MAP (ras_offset + (unsigned short) fgetc (pltin)*256+fgetc(pltin)),
					     rasterline[pos + j + jj] = (unsigned short) GREY_MAP (ras_offset * ((ibyte & (001 << (7 - j))) != 0))
				 );
			    }
			    else
			    {
/*fprintf(stderr,"in this read 4 \n");*/
				READ_RASTER (
					     rasterline[pos + j] = (unsigned short) COLOR_MAP (ras_offset + (unsigned short) fgetc (pltin)),
					     rasterline[pos + j] = (unsigned short) COLOR_MAP (ras_offset + (unsigned short) fgetc (pltin)*256+fgetc(pltin)),
					     rasterline[pos + j + jj] = (unsigned short) COLOR_MAP (ras_offset * ((ibyte & (001 << (7 - j))) != 0))
				 );
			    }
			    pos += num_byte;
			}
...
/*              j,(int) ((j - xru_min) * xrasmult),outraster2[j - xr_min],xrasmult);*/
			...
			if (dither_it)
			{
			    dithline ((unsigned char *)outraster2, (unsigned char *)outraster, xr_max - xr_min, yr_max - 1 - i, dither);
			}
			/* Dumb forms */
 ...
 */
	    ...
	    if (red_fm > 1.0f)
		red_fm = 1.0f;
	    red_fm = (float) pow (red_fm, redpow);
	    ...
	    if (green_fm > 1.0f)
		green_fm = 1.0f;
	    green_fm = (float) pow (green_fm, greenpow);
	    ...
	    if (blue_fm > 1.0f)
		blue_fm = 1.0f;
	    blue_fm = (float) pow (blue_fm, bluepow);

/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
/* Header for class edu_stanford_sep_seplib_corelibs_sep_Putch */

#ifndef _Included_edu_stanford_sep_seplib_corelibs_sep_Putch
#define _Included_edu_stanford_sep_seplib_corelibs_sep_Putch
#ifdef __cplusplus
extern "C" {
#endif
/*
 * Class:     edu_stanford_sep_seplib_corelibs_sep_Putch
 * Method:    putch
 * Signature: (Ljava/lang/String;Ljava/lang/String;[I)I
 */
JNIEXPORT jint JNICALL Java_edu_stanford_sep_seplib_corelibs_sep_Putch_putch__Ljava_lang_String_2Ljava_lang_String_2_3I
  (JNIEnv *, jclass, jstring, jstring, jintArray);

/*
 * Class:     edu_stanford_sep_seplib_corelibs_sep_Putch
 * Method:    putch
 * Signature: (Ljava/lang/String;Ljava/lang/String;[F)I
 */
JNIEXPORT jint JNICALL Java_edu_stanford_sep_seplib_corelibs_sep_Putch_putch__Ljava_lang_String_2Ljava_lang_String_2_3F
  (JNIEnv *, jclass, jstring, jstring, jfloatArray);

/*
 * Class:     edu_stanford_sep_seplib_corelibs_sep_Putch
 * Method:    putch
 * Signature: (Ljava/lang/String;Ljava/lang/String;[D)I
 */
JNIEXPORT jint JNICALL Java_edu_stanford_sep_seplib_corelibs_sep_Putch_putch__Ljava_lang_String_2Ljava_lang_String_2_3D
  (JNIEnv *, jclass, jstring, jstring, jdoubleArray);

/*
 * Class:     edu_stanford_sep_seplib_corelibs_sep_Putch
 * Method:    putch
 * Signature: (Ljava/lang/String;Ljava/lang/String;[B)I
 */
JNIEXPORT jint JNICALL Java_edu_stanford_sep_seplib_corelibs_sep_Putch_putch__Ljava_lang_String_2Ljava_lang_String_2_3B
  (JNIEnv *, jclass, jstring, jstring, jbyteArray);

#ifdef __cplusplus
}
#endif
#endif

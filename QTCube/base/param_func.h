...
class param_func {
  public:
    ...
    virtual int *get_ints(const char *arg, int nmax) const {
      if(strcmp(arg,"")!=0 && nmax==0){;};
      int *n=new int[1]; return n; }
    virtual int *get_ints(const char *arg, int nmax, int *def) const {
      if(strcmp(arg,"")!=0 && nmax==0 && def==0) {;};
     int *n=new int[1]; return n; }
    virtual bool is_input_file(const std::string tag)const{if(strcmp(tag.c_str(),"")!=0){;};return false;}
    virtual bool is_inout_file(const std::string tag)const{if(strcmp(tag.c_str(),"")!=0){;};return false;}

...
void pick_plane_new::del_pick(std::shared_ptr<pick_new>pk){
   picks.erase(pk);
//   fprintf(stderr,"new size is %d \n",(int)picks.size());
}
void pick_plane_new::add_pick(std::shared_ptr<pick_new>pk){
...
void pick_planes_new::add_pick(std::shared_ptr<pick_new>pk){
...
void pick_planes_new::del_pick(std::shared_ptr<pick_new>pk){
  for(std::list<pick_plane_new>:: iterator p=planes.begin(); p!=planes.end(); p++){
    if((*p).in_plane(pk->iloc)) {
       (*p).del_pick(pk);
    }
...
void pick_planes_new::add_plane(int *iloc,std::shared_ptr<picks_vec>pks){
...
bool pick_planes_new::map_exists( int *iloc){
  for(std::list<pick_plane_new>::iterator it=planes.begin(); it!=planes.end(); ++it){
    if(it->same_plane(iloc)) {
               planes.splice(planes.begin(),planes,it);
     return true;
    }
 }
 return false;
...
std::shared_ptr<picks_vec>pick_planes_new::return_picks(int *iloc){
     for(std::list<pick_plane_new>::iterator it=planes.begin(); it!=planes.end(); ++it){
        if(it->same_plane(iloc)) {
          planes.splice(planes.begin(),planes,it);

           return planes.begin()->return_picks();
        }
    }
    assert(1==2);
}
std::shared_ptr<picks_vec>pick_plane_new::return_picks(){
  std::shared_ptr<picks_vec>p (new picks_vec());
  for( auto pk=picks.begin(); pk!=picks.end(); ++pk){
     p->add_pick(*pk);
  }
  return p;
}

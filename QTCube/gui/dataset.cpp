...
std::vector<float> dataset::return_axis_value(int iax,std::vector<long long> positions){
...
void dataset::set_display_axis(int iax,QString x){
 if(!x.contains("grid"))   axes_display[iax]=io->return_header_index(x);
}
...
void dataset::build_conv(){
  ...
  for(int ipt=0; ipt < (int) clip.size(); ipt++){
    ...
    for(int i=std::max(0,(int)(bpct+.5)); i <= std::min(255,(int)(epct+.5)); i++){
  ...
  std::vector<QString> com; com.push_back(QString::number(inum)); 
  com.push_back("clip");
